...
public class BibliotecarioView extends EntityModelView {
	...
	private BibliotecarioTableModel autorModel = new BibliotecarioTableModel(
			BibliotecarioTableModel.dados, BibliotecarioTableModel.colunas);
		...
		if (insertBut == null) {
			...
			insertBut.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					bibliotecario.setNome(getNomeField().getText());
					...
					try {
						id = bibliotecarioController.getId(bibliotecario);
					} catch (BusinessException e1) {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (x = 0; x < jTable1.getRowCount(); x++) {
						...
						if (oi == true) {
							...
							try {
								bibliotecarioController
										.excluirBibliotecario(bibliotecario);
							} catch (BusinessException e1) {
		...
		if (updateBut == null) {
			...
			updateBut.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						bibliotecarioController
								.alterarBibliotecario(bibliotecario);
					} catch (BusinessException e1) {
					...
					bibliotecario.setId(z);
					String[] linha = new String[] { "" + bibliotecario.getId(),
							bibliotecario.getNome() };
	...
	private BibliotecarioView getThis() {
		return this;
	}
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (x = 0; x < jTable1.getRowCount(); x++) {
						...
						if (oi == true) {
							...
							bibliotecario.setId(id);
							try {
								bibliotecarioController.select(bibliotecario,
										getThis());
							} catch (BusinessException e1) {
	...
	private void initialize() {
		...
		this.setTitle("BibliotecarioView");
