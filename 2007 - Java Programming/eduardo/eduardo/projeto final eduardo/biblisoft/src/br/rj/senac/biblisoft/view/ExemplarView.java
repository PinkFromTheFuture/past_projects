...
public class ExemplarView extends EntityModelView {
	...
	private ExemplarController exemplarController = new ExemplarController(); // @jve:decl-index=0:
		...
		if (insertBut == null) {
			...
			insertBut.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					livro.setNome(getLivroChoice().getSelectedItem());

					try {
						exemplar.setIdLivro(livroController.getId(livro));
					...
					try {
						...
						exemplarController.inserirExemplar(exemplar);
					} catch (BusinessException e1) {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (x = 0; x < jTable1.getRowCount(); x++) {
						...
						if (oi == true) {
							...
							exemplar.setId(id);
							try {
								exemplarController.excluirExemplar(exemplar);
							} catch (BusinessException e1) {
		...
		if (updateBut == null) {
			...
			updateBut.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					try {
						exemplarController.alterarExemplar(exemplar);
					} catch (BusinessException e1) {
				...
				public void actionPerformed(java.awt.event.ActionEvent e) {
					...
					for (x = 0; x < jTable1.getRowCount(); x++) {
						...
						if (oi == true) {
							...
							exemplar.setId(id);
							try {
								exemplarController.select(exemplar, getThis());
							} catch (BusinessException e1) {
